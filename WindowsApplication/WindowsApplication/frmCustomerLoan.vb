Public Class frmCustomerLoan
    ...
    Private Sub cmdSave_Click(sender As Object, e As EventArgs) Handles cmdSave.Click
        If CheckEmptyfieldtxt(txtCuLNo, "This Data couldn't be saved because Customer Loan No was empty. Please Check it and try again.") = False Then
            Exit Sub
        ElseIf CheckEmptyfieldtxt(txtCuLAmount, "This Data couldn't be saved because Customer Loan Amount was empty. Please check it and try again.") = False Then
            Exit Sub
        ElseIf CheckEmptyfieldcmb(cmbCuName, "Customer Name යන field එක හිස්ව පවතියි. කරුණාකර එය සම්පුර්ණ කරන්න. ") = False Then
            Exit Sub
        ElseIf CheckExistDatacmb(cmbCuName, "Select CuName from Customer Where CuName='" & cmbCuName.Text & "'", "Customer ව Database එක තුල සොයා ගැනීමට නොහැකි විය. කරුණාකර නැවත පරික්ෂා කරන්න.", False) = False Then
            Exit Sub
        End If
        Dim CuNo As Integer
        CMD = New OleDb.OleDbCommand("Select CuNo from Customer Where CuName='" & cmbCuName.Text & "'", CNN)
        DR = CMD.ExecuteReader
        If DR.HasRows = True Then
            CuNo = DR("CuNo")
        End If
        Select Case cmdSave.Text
            Case "Save"
                If CheckExistDatatxt(txtCuLNo, "SELECT CULNO FROM CUSTOMERLOAN WHERE CULNO = " & txtCuLNo.Text & ";", "Something was wrong. This Customer Loan No is already exist in the database. Please Check it and try again. Otherwise you can contact a software developer.", True) = True Then
                    Exit Sub
                End If
    ...
    Private Sub cmdSearch_Click(sender As Object, e As EventArgs) Handles cmdSearch.Click
        ...
        If txtSearch.Text <> "" Then
            Select Case cmbFilter.Text
                ...
                Case "All"
                    x = "Where CUL.CuLNo like '%" & txtSearch.Text & "%' or CuLDate like '%" & txtSearch.Text & "%' or CUL.CuNo like '%" & txtSearch.Text & "%' or CuName like '%" & txtSearch.Text & "%' or CuTelNo1 like '%" & txtSearch.Text & "%' or CuTelNo2 like '%" & txtSearch.Text & "%' or CuTelNo3 like '%" & txtSearch.Text & "%' or CUL.SaNo like '%" & txtSearch.Text & "%' or SaDate like '%" & txtSearch.Text & "%' or CUL.DNo like '%" & txtSearch.Text & "%' or DDate like '%" & txtSearch.Text & "%' or Status like '%" & txtSearch.Text & "%'"
            End Select
        Else
            x = " Order by CUL.CuLNo"
        End If
        Dim da As New OleDb.OleDbDataAdapter("SELECT CuL.CuLNo as [Customer Loan No],CuLDate as [Customer Loan Date],CuL.CuNo as [No],CuName as [Name],CuTelNo1 as [Telephone No 1],CuTelNo2 as [Telephone No 2],CuTelNo3 as [Telephone No 3],CuL.SaNo as [Sale No],SaDate as [Sale Date], CuL.DNo as [Deliver No], DDate as [Deliver Date], Status,CuLRemarks as [Remarks] from (((CustomerLoan CUL INNER JOIN CUSTOMER CU ON CU.CUNO = CUL.CUNO) LEFT JOIN SALE SA ON SA.SANO = CUL.SANO) LEFT JOIN DELIVER D ON D.DNO = CUL.DNO) " & x & ";", CNN)
        da.Fill(dt)
