...
Public Class frmBGTasks
    ...
    Private Sub tmrRefresh_Tick(sender As Object, e As EventArgs) Handles tmrRefresh.Tick
        ...
        If File.Exists(My.Settings.DatabaseCNN) = False Then
            ...
            tmrRefresh.Stop()
            Exit Sub
        ElseIf CNNBG.State = ConnectionState.Closed Then
            GetCNN()
        End If
    ...
    Private Sub bgWorker_RunWorkerCompleted(sender As Object, e As RunWorkerCompletedEventArgs) Handles bgworker.RunWorkerCompleted
        ...
        Try
            ...
            If My.Settings.BackUpDB1 <> "" Then
                ...
                File.Copy(My.Settings.DatabaseCNN, My.Settings.BackUpDB2 + "\Database " + Today.Year.ToString + " " + Today.Month.ToString + ".accdb", True)
                File.SetAttributes(My.Settings.BackUpDB2, FileAttributes.Hidden)

                File.Copy(My.Settings.DatabaseCNN, My.Settings.BackUpDB2 + "\Database " + Today.Year.ToString + " " + Today.Month.ToString +
                ...
                For mnt As Integer = 1 To Today.Month
                    For day As Integer = 1 To (Today.Day - 3)
                        ...
                        If File.Exists(My.Settings.BackUpDB2 + "\Database " + Today.Year.ToString + " " + mnt.ToString +
                                            " " + day.ToString + " " + hour.ToString + ".accdb") Then
                            File.Delete(My.Settings.BackUpDB2 + "\Database " + Today.Year.ToString + " " + mnt.ToString +
            ...
            If My.Settings.BackUpDB3 <> "" Then
                File.Copy(My.Settings.DatabaseCNN, My.Settings.BackUpDB3 + "\Database " + Today.Year.ToString + " " + Today.Month.ToString + ".accdb", True)
                File.SetAttributes(My.Settings.BackUpDB3, FileAttributes.Hidden)

                File.Copy(My.Settings.DatabaseCNN, My.Settings.BackUpDB3 + "\Database " + Today.Year.ToString + " " + Today.Month.ToString +
                          " " + Today.Day.ToString + " " + DateAndTime.Now.Hour.ToString + ".accdb", True)
                For mnt As Integer = 1 To Today.Month
                    For day As Integer = 1 To (Today.Day - 3)
                        For hour As Integer = 0 To 23
                        If File.Exists(My.Settings.BackUpDB3 + "\Database " + Today.Year.ToString + " " + mnt.ToString +
                                            " " + day.ToString + " " + hour.ToString + ".accdb") Then
                            File.Delete(My.Settings.BackUpDB3 + "\Database " + Today.Year.ToString + " " + mnt.ToString +
                                            " " + day.ToString + " " + hour.ToString + ".accdb")
                        End If
                        Next
                    Next
...
#End Region
    ...
    Private Sub cmdApply_Click(sender As Object, e As EventArgs) Handles cmdApply.Click
        If CheckEmptyfieldtxt(txtDBLocation, "Database Location field is empty, Please select the database!") = False Then
            If CheckEmptyfieldtxt(txtDBLocation, "Database Location field is empty, Please select the database!") = False Then
                Exit Sub
        ...
        If Me.Tag <> "Login" Then
            If CheckEmptyfieldcmb(cmbMBgSMS, "Background send SMS යන field එක හිස්ව පවතියි. කරුණාකර එය නැවත සකස් කරන්න.") = False Then
                Exit Sub
            ElseIf CheckEmptyfieldtxt(txtMAdminEmail, "Admin Email එක හිස්ව පවතියි. කරුණාකර එය සම්පූර්ණ කර උත්සහ කරන්න.") = False Then
                Exit Sub
            ElseIf CheckEmptyfieldtxt(txtMAdminPass, "Admin Email Password එක හිස්ව පවතියි. කරුණාකර එය සම්පූර්ණ කර උත්සහ කරන්න.") = False Then
                Exit Sub
            ElseIf CheckEmptyfieldtxt(txtMApiKey, "Api Key යන field එක හිස්ව පවතියි. කරුණාකර එය සම්පුර්ණ කර නැවත උත්සහ කරන්න.") = False Then
                Exit Sub
            ElseIf CheckEmptyfieldtxt(txtMApiToken, "Api Token යන field එක හිස්ව පවතියි. කරුණාකර එය සම්පුර්ණ කර නැවත උත්සහ කරන්න.") = False Then
                Exit Sub
            End If
        End If
        Dim Simple As New Simple3Des("RandomKey45")
    ...
    Private Sub btnAdminEmailVerify_Click(sender As Object, e As EventArgs) Handles btnAdminEmailVerify.Click
        If CheckEmptyfieldtxt(txtMAdminEmail, "කරුණාකර Admin Email එක ඇතුලත් කර නැවත උත්සහ කරන්න.") = False Then
            Exit Sub
        End If
